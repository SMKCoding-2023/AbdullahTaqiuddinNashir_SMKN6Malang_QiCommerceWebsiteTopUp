...
import { useCategoryStore } from '@/stores/category'; // Update the path accordingly
  ...
  <div v-show="isShow" class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-xl w-full absolute top-14">
    <ul class="py-2 text-sm text-gray-700">
      ...
      <li v-for="(category, index) in categoryStore.categories" :key="index">
        <span class="block px-4 py-2 hover-bg-primary hover-text-white transition duration-200 cursor-pointer" @click="$emit('selectedCategory', category.name)">{{ category.name }}</span>
      </li>
    </ul>
  </div>
  </div>
